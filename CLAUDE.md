...
- **Key bindings**, handled on the row itself: `ArrowUp`/`ArrowDown` move focus one row, clamped at the boundaries (no wraparound, matching `moveColumnBy`'s convention); `Home`/`End` jump to the first/last visible row; `Space` calls `toggleRowSelection(row, shiftKey)` (only when `selectable`); `Enter` fires `onRowClick`/`rowClick` (only when it's set), reusing the exact same handler the row's click already calls. `Shift+ArrowUp/Down/Home/End` additionally call `toggleRowSelection(target, true)` before moving focus — i.e. a shift-arrow-key press is treated exactly like a shift-click on the row it's moving _to_, so it composes for free with the existing anchor/`selectRange` logic (see "Row selection" above) with no new core primitive.
