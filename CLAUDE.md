...
Rows use a **roving tabindex**: exactly one data row is a Tab stop at a time (`tabIndex`/`tabindex` `0`, the rest `-1`), arrow keys move it, and rows only join the tab sequence at all when the table is actually interactive (`selectable` or `onRowClick` set) — a purely display-only table has no row-level tab stops. This only covers the table body; toolbar buttons, dropdowns, and filter checkboxes are already native focusable elements and needed no changes.

- **`getVisibleRows(groups, collapsedGroups)`** (core) flattens `groupData`'s result into the rows actually rendered — skipping a collapsed group's rows entirely, the same condition each adapter's render already applies. This is the Up/Down/Home/End order: a collapsed group's rows are simply not valid arrow-key targets, and expanding/collapsing a group changes what's reachable, exactly as it changes what's on screen.
- The currently-tabbable row is tracked as `focusedRow: TRow | null` — ephemeral UI state by object identity, same category as `selectionAnchor`/`hoveredRow`, not part of `TableViewState`. Each render derives `effectiveFocusRow = focusedRow && visibleRows.includes(focusedRow) ? focusedRow : visibleRows[0]`, the same "stale state still renders something sane" fallback used for `filterActiveKey`/`visibleCols` — so a focused row that scrolls out of view (sorted/filtered/grouped away) self-heals to the first visible row instead of leaving no tab stop at all.
- **Key bindings**, handled on the row itself: `ArrowUp`/`ArrowDown` move focus one row; when pagination is active, moving past the current page's first/last row crosses into the previous/next page instead of clamping there (see "Crossing page boundaries" below). `Home`/`End` jump to the first/last visible row _of the current page_; `Ctrl+Home`/`Ctrl+End` (or `Cmd+Home`/`Cmd+End` on Mac) jump to the true first/last row across _all_ pages, matching the spreadsheet/ARIA-grid convention. `Space` calls `toggleRowSelection(row, shiftKey)` (only when `selectable`); `Enter` fires `onRowClick`/`rowClick` (only when it's set), reusing the exact same handler the row's click already calls. `Shift+ArrowUp/Down/Home/End` (and `Shift+Ctrl+Home/End`) additionally call `toggleRowSelection(target, true)` before moving focus — i.e. a shift-arrow-key press is treated exactly like a shift-click on the row it's moving _to_, so it composes for free with the existing anchor/`selectRange` logic (see "Row selection" above) with no new core primitive; this also means a shift-crossing selection already spans the page boundary correctly, since `toggleRowSelection`'s range computation runs over `processedData` (the full order), not the current page's slice.
- The row checkbox gets `tabIndex={-1}` / `tabindex="-1"` unconditionally when `selectable` — with row-level keyboard interaction (`Space`/`Shift+Arrow`) covering selection, a separately-tabbable checkbox would just be a redundant second Tab stop per row. Clicking it with a mouse is unaffected; only its presence in sequential Tab order changes.
...
- Selected rows get `aria-selected` (`selectable` only) so the selection state is announced, matching native listbox/grid conventions, without adopting a full ARIA grid (`role="grid"`, per-cell focus) — this table's interactions are all row-level (selection, click, range), never per-cell, so a lighter row-level model was chosen over the heavier grid pattern.

**Crossing page boundaries**: `visibleRows`/`_visibleRows` only covers the _current_ page — it's built from `groupedData`, which is itself `groupData(pagedData, ...)`. Reaching a row on another page (an ArrowUp/ArrowDown at the edge, or Ctrl+Home/Ctrl+End) needs that same pipeline re-run for a different page, on demand: each adapter has a `visibleRowsForPage(p)` helper — `getVisibleRows(groupData(paginateData(processedData, p, pageSize), groupBy, columns, emptyLabel), collapsedGroups)` — called only when a key press actually needs to look past the current page (not memoized; it's cheap and rare). `Ctrl`/`Cmd+Home` targets page `1`, `Ctrl`/`Cmd+End` targets `numPages`, taking the first/last value returned; a boundary ArrowDown targets `page + 1`'s first row, ArrowUp targets `page - 1`'s last row.

Per-adapter wiring:

- **React** — `focusedRow` (`useState`) plus a `rowRefs = useRef(new Map<TRow, HTMLTableRowElement>())` populated via each row's callback `ref`. Moving focus calls `.focus()` on the target's ref synchronously inside the key handler (not via an effect) — the row is already mounted, so this is simpler than waiting a render cycle. `onFocus` (bubbling in React, unlike the native `focus` event) keeps `focusedRow` in sync when focus lands on a row via Tab or via a mouse click on its (now `tabIndex={-1}`) checkbox. Crossing a page boundary calls `setPage`, whose state update re-renders asynchronously — the row to focus can't be reached via a ref until that render commits, so it's stashed in a `pendingFocusRow` ref and a `useEffect` keyed on `[page]` calls `focusRow` once the effect fires (i.e. after the new page's rows are mounted).
- **Vue** — the same shape: `focusedRow` (`ref`), a plain (non-reactive) `rowRefs` `Map` populated via a function `:ref`, and `@focusin` (which, unlike `@focus`, bubbles natively) instead of React's auto-bubbling `onFocus`. The inline template assignment `focusedRow = row` hits a `vue-tsc` generic/ref-distribution error under this SFC's `generic="TRow extends object"` param, so it's a named `setFocusedRow(row)` function instead, following the same pattern as `handleRowClick`. Crossing a page boundary mirrors React's `pendingFocusRow` + effect approach, but as a plain closure variable plus `watch(page, ..., { flush: 'post' })` — `flush: 'post'` (rather than the default `'pre'`) is what makes the callback run after the DOM actually updates, so `rowRefs` already holds the new page's row elements by the time it fires.
- **Vanilla** — `focusedRow` is a closure var. `_visibleRows` is recomputed every `render()` alongside `_groupedData`; a new delegated `keydown` container listener (`handleKeyDown`) resolves the target row via `e.target.closest('.dt-tr[data-proc-idx]')`, the same ancestor-lookup style `handleClick` already uses, so it fires correctly whether the keydown originated on the row itself or bubbled up from its (now `tabIndex="-1"`) checkbox. The checkbox click handler and the keyboard Space/Shift+Arrow path share one `applyRowSelectionToggle(row, shiftKey)` helper (extracted from what was inline in the `toggle-row-select` case) so the two entry points can't drift. Because every interaction here re-renders via `innerHTML` (destroying the previously-focused DOM node), `render()` gained a second focus-restore step alongside the existing `data-focus-key` one: it captures whether a `.dt-tr` had focus before the rebuild, then re-`.focus()`es the new node matching `effectiveFocusRow` by object identity afterward — without it, every arrow-key press would drop focus to `<body>` and silently stop working after one keystroke. Since `render()` runs synchronously (no framework re-render delay), crossing a page boundary needs no separate pending-focus mechanism: `moveFocus(target, targetPage)` just sets `focusedRow`/`page` and calls `render()` directly, and the existing focus-restore step above picks up the new row by identity on that same call.
